...
-export([init/1, wait/2, init2/2, map/2, reduce/2, terminate/3]).
...
wait({job, NumWork, MapFun, RedFun, Initial, Data}, []) ->
    {next_state, init2, {initialize, NumWork, MapFun, RedFun, Initial, Data}, 0}.

init2(timeout, {initialize, NumWork, MapFun, RedFun, Initial, Data}) ->
    ...
    {next_state, map, {Reducer, NumWork-1, RedFun, Initial, []}}.
...
map({mapper_done, _Worker, Result}, {Reducer, MapperCount, RedFun, Initial, Results}) ->
    ...
    case MapperCount of
        1 ->
            ...
            {next_state, reduce, []};
        _ ->
            {next_state, map, {Reducer, MapperCount-1, RedFun, Initial, append(Result, Results)}}
    end.
    %io:format("Result: ~p~n", [Result]),
    %{next_state, wait, []}.

reduce({reducer_done, _Worker, Result}, []) ->
    io:format("Result: ~p~n", [Result]),
    {next_state, wait, []}.
